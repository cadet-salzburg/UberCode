...
void QGlTextureImage::paintGL()
{
	...
	std::cout << "PaintGL is Called" << std::endl;
}
void QGlTextureImage::paintEvent(QPaintEvent*)
{
//	QPainter p(this);
 ...
 //   p.SetRenderHint(QPainter::SmoothPixmapTransform, 1);
  //  p.drawImage(this->rect(), m_Image);
	//paintGL();
	std::cout << "Paint Event is Called" << std::endl;
}
...
{
//	std::cout << "Update Texture is called" << std::endl;
	...
	if ( ( width > 0 ) && ( height > 0 ) )
	{
		m_Image.loadFromData( (unsigned char *) data, width*height*channels);//, (const char *) QImage::Format_RGB888 );
